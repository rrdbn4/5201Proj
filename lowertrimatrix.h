...
// Course:     CS 5201
// Purpose:    class declaration for the lowertrimatrix class
///////////////////////
...
////////////////////////
template <class T>
class LowerTriMatrix : public Matrix<T>
{
  private:
  ...
  public:
    ...
    LowerTriMatrix<T> operator+(const LowerTriMatrix<T>& rhs) const;
    ...
    LowerTriMatrix<T>& operator=(const LowerTriMatrix<T>& input);
    ...
    template <class N>
    friend LowerTriMatrix<N> operator*(const N& scalar, const LowerTriMatrix<N>& mat);
    ...
    template <class N>
    friend Vector<N> operator*(const Vector<N>& vec, const LowerTriMatrix<N>& mat);
    ...
    // Desc: overloaded * operator for vector - matrix multiplication
    template <class N>
    friend Vector<N> operator*(const LowerTriMatrix<N>& mat, const Vector<N>& vec);
    ...
    // Desc:overloaded << operator
    template <class N>
    friend std::ostream& operator<<(std::ostream& stream, const LowerTriMatrix<N>& input);
    ...
    // Desc:overloaded >> operator
    template <class N>
    friend std::istream& operator>>(std::istream& stream, LowerTriMatrix<N>& input);
};
