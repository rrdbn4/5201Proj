...
// Course:     CS 5201
// Purpose:    class declaration for the TriDiagmatrix class
///////////////////////
...
class TriDiagMatrix : public Matrix<T>
{
  ...
  public:
    ...
    //Desc: perform the thomas algorithm to solve the system of equations
    ...
    TriDiagMatrix<T> operator+(const TriDiagMatrix<T>& rhs) const;
    ...
    TriDiagMatrix<T>& operator=(const TriDiagMatrix<T>& input);
    ...
    template <class N>
    friend TriDiagMatrix<N> operator*(const N& scalar, const TriDiagMatrix<N>& mat);
    ...
    template <class N>
    friend Vector<N> operator*(const Vector<N>& vec, const TriDiagMatrix<N>& mat);
    ...
    // Desc: overloaded * operator for vector - matrix multiplication
    template <class N>
    friend Vector<N> operator*(const TriDiagMatrix<N>& mat, const Vector<N>& vec);
    ...
    // Desc:overloaded << operator
    template <class N>
    friend std::ostream& operator<<(std::ostream& stream, const TriDiagMatrix<N>& input);
    ...
    // Desc:overloaded >> operator
    template <class N>
    friend std::istream& operator>>(std::istream& stream, TriDiagMatrix<N>& input);
};
