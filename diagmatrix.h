...
// Course:     CS 5201
// Purpose:    class declaration for the diagmatrix class
///////////////////////
...
class DiagMatrix : public Matrix<T>
{
  ...
  public:
    ...
    //Desc: perform simple math to solve the system of equations
    ...
    DiagMatrix<T>& operator=(const DiagMatrix<T>& input);
    ...
    template <class N>
    friend DiagMatrix<N> operator*(const N& scalar, const DiagMatrix<N>& mat);
    ...
    template <class N>
    friend Vector<N> operator*(const Vector<N>& vec, const DiagMatrix<N>& mat);
    ...
    // Desc: overloaded * operator for vector - matrix multiplication
    template <class N>
    friend Vector<N> operator*(const DiagMatrix<N>& mat, const Vector<N>& vec);
    ...
    // Desc:overloaded << operator
    template <class N>
    friend std::ostream& operator<<(std::ostream& stream, const DiagMatrix<N>& input);
    ...
    // Desc:overloaded >> operator
    template <class N>
    friend std::istream& operator>>(std::istream& stream, DiagMatrix<N>& input);
};
