...
// Course:     CS 5201
// Purpose:    class declaration for the uppertrimatrix class
///////////////////////
...
////////////////////////
template <class T>
class UpperTriMatrix : public Matrix<T>
{
  private:
  ...
  public:
    ...
    UpperTriMatrix<T>& operator=(const UpperTriMatrix<T>& input);
    ...
    template <class N>
    friend UpperTriMatrix<N> operator*(const N& scalar, const UpperTriMatrix<N>& mat);
    ...
    template <class N>
    friend Vector<N> operator*(const Vector<N>& vec, const UpperTriMatrix<N>& mat);
    ...
    // Desc: overloaded * operator for vector - matrix multiplication
    template <class N>
    friend Vector<N> operator*(const UpperTriMatrix<N>& mat, const Vector<N>& vec);
    ...
    // Desc:overloaded << operator
    template <class N>
    friend std::ostream& operator<<(std::ostream& stream, const UpperTriMatrix<N>& input);
    ...
    // Desc:overloaded >> operator
    template <class N>
    friend std::istream& operator>>(std::istream& stream, UpperTriMatrix<N>& input);
};
